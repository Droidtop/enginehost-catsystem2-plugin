...
struct cs2_files {
    char root[1024];
    /* NULL for a real game_root; set for an isolated launch, where every
...
/* Same construction, over a host broker instead of a real game_root path (docs/engine-sandbox.md). */
cs2_files *cs2_files_open_via_broker(const cs2_broker *broker) {
    ...
    if (total < 0) {
        free(names);
        cs2_set_error("the game folder cannot be listed");
        return NULL;
    }
    cs2_files *files = calloc(1, sizeof *files);
    if (files == NULL) {
    ...
    files->broker = broker;
    for (int i = 0; i < total && files->count < MAX_ARCHIVES; i++) {
        if (!has_int_suffix(names[i])) continue;
        slot *entry = &files->archives[files->count];
    ...
    cs2_game_key_read_via_broker(broker, &files->key);
    return files;
}
...
cs2_kif *cs2_files_archive(cs2_files *files, const char *archive_name) {
    ...
    for (size_t i = 0; i < files->count; i++) {
        ...
        if (entry->archive == NULL && !entry->tried) {
            entry->tried = 1;
            if (files->broker != NULL) {
                int fd = files->broker->open_read(files->broker->ctx, entry->name);
                entry->archive = fd < 0 ? NULL : cs2_kif_open_fd(fd, entry->name, &files->key);
            } else {
...
static int read_loose(const cs2_files *files, const char *relative, cs2_bytes *out) {
    ...
    if (files->broker != NULL) {
        int fd = files->broker->open_read(files->broker->ctx, relative);
        if (fd < 0) return -1;
        return cs2_read_file_fd(fd, MAX_LOOSE, out);
    }
    char path[2048];
